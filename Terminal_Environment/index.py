...
from sharepoint_documents import get_user_email_by_token, get_sharepoint_chunk
...
# files = get_onedrive_file_names(access_token)
# print(files)
# file0_name, file0_id = files[0]
# print(f"does user have access to file[0]: {user_has_access_to_file(access_token, file0_id)}")

# start Q/A session after athenticating
print("\n\n-------------------------------------------------------")
print(f"Welcome to ChairGPT, {user_email}! (Please enter '/exit' to quit)")
...
while True:
  ...
  qa_log = generate_qa_log(user_email, question, response, similar_information, metadata_user, metadata_bot)
