...
from helpers import open_file, save_file, get_datetime
from sharepoint_documents import get_sharepoint_chunk, user_has_access_to_file
...
def retrieve_accessible_similar_information(vdb, sharepoint_auth_token, user_email:str, vector, k=6, threshold=2, max_tries=3, info_separator=" -- "):
    ...
    num_accessible_documents = 0
    accessible_documents = ""
    tries = 0

    # continue to query until meeting threshold or maximum tries are met
    while (num_accessible_documents < threshold and tries < max_tries):
        ...
        for match in matches:
            user_has_access = user_has_access_to_file(sharepoint_auth_token, match['metadata']['document_id'])
            print(user_has_access)
            if user_has_access:
                doc_contents = get_sharepoint_chunk(
...
def generate_qa_log( user_email:str, prompter_message:str, responder_message:str, info:str, prompter_metadata, responder_metadata):
    """
